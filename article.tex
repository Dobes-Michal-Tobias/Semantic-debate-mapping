...
% ══════════════════════════════════════════════════════════════════════
\title{%
  \textbf{Mapování sémantické konvergence a lexikální diverzity\\ v~předvolebních debatách 2025}%
  \thanks{Zdrojový kód a data: \url{https://github.com/Dobes-Michal-Tobias/Semantic-debate-mapping}}%
}

\author{%
  \textbf{Michal Tobiáš Dobeš} \\[1ex]
  {\normalsize Katedra obecné lingvistiky, Univerzita Palackého v~Olomouci} \\
  {\small Předmět: \textbf{KRAD} --- Kritická analýza diskurzu}%
}
...
televize (podzim~2025). Metodologický rámec propojuje sentence embeddingy
(model \modelname{}), redukci dimenzí
algoritmem UMAP, lexikální metriky (Type-Token Ratio, Maas index) s pokročilou strojovou klasifikací promluv pomocí velkých jazykových modelů, umožňující detekci sentimentu, míry agresivity či dominantních rétorických rámců. Výsledky vizualizované prostřednictvím sémantické mapy,
...
Předvolební kampaň před volbami do Poslanecké sněmovny ČR v~říjnu 2025 představovala
...
analytickou pipeline, která transformuje surové textové přepisy debat na strukturované
...
diverzní výroky) poskytuje zajímavý kontrast.
...
Analytická pipeline sestává z šesti fází, z~nichž pátá (LLM obohacení)
...
Tokenizace je implementována jazykově nezávislým způsobem bez ohledu na gramatická či morfologická specifika daného jazyka
(převod na malá písmena, segmentace na mezerách).
...
Jako záložní metoda je implementována PCA z~knihovny scikit-learn, která se
...
\begin{enumerate}
  ...
  diskurz témat sdílí značnou část sémantického prostoru
  napříč ideologickým spektrem.

  \item \textbf{Sémantická pozice moderátora.} Promluvy moderátora vykazují výraznou tendenci ke~shlukování v~rámci hlavního sémantického clusteru, což reflektuje jeho věcně neutrální a organizační roli. Přesto je u~těchto promluv patrná tendence k~formování vlastních specifických podshluků; tato diskurzivní distinkce pravděpodobně odráží funkční odlišnost metajazykových a procedurálních prvků od~věcné argumentace politických aktérů.
  
  \item \textbf{Okrajové shluky.} V~horní části projekce i na pravo od hlavního dominantního clusteru lze
...
\begin{enumerate}
  ...
  s~vyšší mírou lexikální repetice. Může jít o styl
...
\begin{itemize}
  \item \textbf{Dominance moderátora v zahraniční politice.} Výrazným rysem heatmapy je anomální koncentrace promluv moderátora v bloku „Zahraniční politika“ (32 \%), což ostře kontrastuje s jeho minimálním zapojením v duelových formátech (2--3 \%). Tato hodnota může indikovat buď rozsáhlé moderátorské expozice v daném tématu, nebo potenciální nesrovnalost v datovém souboru.

  \item \textbf{Strategická stabilita hnutí ANO.} Projevy hnutí ANO vykazují vysokou míru tematické stability v rozmezí 6--8 \% u většiny agend. Signifikantní propad je však patrný v tématech bezpečnosti a obrany (4 \%) a zdravotnictví (4 \%).

  \item \textbf{Tematické priority Motoristů.} Subjekt vykazuje cílené zaměření na oblasti „Společnost a výzvy“ a „Digitalizace státu“, kde dosahuje shodně 8 \% svého celkového slovního rozsahu, což je mírně nadprůměrná hodnota ve srovnání s jejich ostatními agendami.

  \item \textbf{Digitalizační akcent Pirátů.} Agenda Pirátů je rozdělena do dvou úrovní: primární skupina témat vykazuje podíl kolem 7 \%, zatímco sekundární agendy (např. justice, vztahy s EU) se pohybují kolem 4--5 \%. Výjimku tvoří téma „Digitalizace státu“ (9 \%), do kterého se Piráti zapojili výrazněji než do většiny ostatních bloků, což patrně souvisí s akcentem na agendu ex-ministra Bartoše.

  \item \textbf{Konfrontační dominance SPD.} Strategie SPD je charakteristická extrémním výkyvem v rámci duelového formátu (14. 9.), kde předseda Okamura generuje 19 \% celkového slovního objemu strany, což výrazně převyšuje podíl v jakémkoliv jiném tematickém bloku.

  \item \textbf{Programová vyváženost koalice SPOLU.} Jako klíčový subjekt tehdejší vládní koalice vykazuje SPOLU nejrovnoměrnější distribuci promluv napříč všemi bloky (většinou 7--8 \%). Tato rovnoměrnost indikuje snahu o komplexní pokrytí celého vládního spektra bez výrazných prioritních či deficitních témat.

  \item \textbf{Vysoká agendační volatilita Stačilo!.} Projevy subjektu Stačilo! vykazují značnou nevyrovnanost; výrazně se soustředí na témata „Vztahy s EU“ a „Zahraniční politika“ (shodně 10 \%), avšak téměř zcela opomíjejí oblasti bezpečnosti (4 \%), vzdělávání (3 \%) či společenských výzev (3 \%).
\end{itemize}
...
politické strany v~předvolebních tématech ve~sdíleném sémantickém prostoru.
...
diskurzu. Pipeline transformuje surový textový přepis na trojici
...
odpovídá) a~srovnání s~korpusy politických debat v~jiných
jazycích středoevropského prostoru.

% ══════════════════════════════════════════════════════════════════════
\section*{Dostupnost dat a~kódu}
\addcontentsline{toc}{section}{Dostupnost dat a~kódu}

Veškerý zdrojový kód analytické pipeline, vstupní data i~výstupní
vizualizace jsou veřejně dostupné v~repozitáři:

\begin{center}
\url{https://github.com/Dobes-Michal-Tobias/Semantic-debate-mapping}
\end{center}

\noindent Repozitář obsahuje soubor \texttt{requirements.txt} s~výčtem
závislostí a~instrukce pro reprodukci všech fází analýzy.
